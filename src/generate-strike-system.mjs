...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'strike', 'Strike', and 'STRIKE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.strike/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.strike/g),
   ...
   * Rename files.
   *
   * Rename files that had strike in their name, such as
   * css/strike.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*strike*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('strike', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-strike-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Strike's files so that we can process them.
    glob('*', {ignore: 'node_modules/**' }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace strike name mentions in files.
      generator.replaceFileContents();
      // Rename files that had strike in their name.
